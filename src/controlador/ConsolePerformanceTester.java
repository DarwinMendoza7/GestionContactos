...
public class ConsolePerformanceTester {
    ...
    //Prueba el tiempo de carga de contactos, comparando versiones concurrente y secuencial
    private void testLoadContacts() {
        ...
        for (int i = 0; i < TEST_ITERATIONS; i++) {
            ...
            System.out.printf("Iteración %d - Concurrente: %d ms | Secuencial: %d ms%n",
                i+1, concurrentTime, sequentialTime);
        }

        printResults("CARGA DE CONTACTOS (Promedio)", 
                    totalConcurrent/TEST_ITERATIONS, 
                    totalSequential/TEST_ITERATIONS);
    }

    //Prueba el tiempo de exportación de contactos a CSV, comparando versiones concurrente y secuencial
    private void testExportContacts() {
        ...
        printResults("EXPORTACIÓN DE CONTACTOS (Promedio)", 
                    totalConcurrent/TEST_ITERATIONS, 
                    totalSequential/TEST_ITERATIONS);
    }
    ...
    //Procesa la lista de contactos de forma secuencial, simulando una operación costosa
    ...
    private String processContactLine(Persona contacto) {
        ...
        return String.join(",",
            ...
            String.valueOf(contacto.isFavorito())
        );
    }
    ...
    private List<Persona> generateTestContacts(int count) {
        ...
        for (int i = 0; i < count; i++) {
            contacts.add(new Persona(
                "Contacto_" + i,
                "600" + (100000 + random.nextInt(900000)),
                "contacto" + i + "@test.com",
                random.nextBoolean() ? "family" : 
                    (random.nextBoolean() ? "friends" : "work"),
                random.nextBoolean()
            ));
        }
        return contacts;
    ...
    // Método estático para preguntar al usuario si desea ejecutar las pruebas de rendimiento
